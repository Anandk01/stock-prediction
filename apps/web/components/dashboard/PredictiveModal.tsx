...
import { 
    X, Search, Activity, Cpu, Clock, Newspaper, Sparkles, 
    ArrowUpRight, ArrowDownRight, RefreshCw, Award, CheckCircle2, 
    AlertCircle, HelpCircle, Layers, TrendingUp, AlertTriangle
} from "lucide-react";
...
export default function PredictiveModal({ isOpen, onClose, accessToken }: PredictiveModalProps) {
    // Search State
    ...
    const fetchStockData = async (symbol: string) => {
        ...
        try {
            ...
            // 1. Fetch current predictions & metrics
    ...
    const triggerIncrementalUpdate = async () => {
        ...
        try {
            ...
            // Artificial delay steps for judging UI effect
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 overflow-y-auto bg-slate-950/80 backdrop-blur-md transition-all">
            {/* Modal Body */}
            <div className="relative w-full max-w-6xl bg-[#090d22] border border-white/10 rounded-[2.5rem] shadow-2xl overflow-hidden text-white flex flex-col max-h-[90vh]">
                
                {/* Header */}
                <div className="p-6 border-b border-white/5 flex justify-between items-center bg-white/[0.01]">
                    <div className="flex items-center gap-3">
                        <div className="p-2.5 bg-cyan-500/25 rounded-2xl border border-cyan-400/30 text-cyan-400">
                            <Cpu size={24} className="animate-spin-slow" />
                        </div>
                        <div>
                            <h2 className="text-2xl font-black tracking-tight">AI Predictive Hub</h2>
                            <p className="text-xs text-gray-500">Incremental Online Learning Platform (River ML & FinBERT)</p>
                        </div>
                    </div>
                    
                    {/* Search Bar */}
                    <form onSubmit={handleSearch} className="hidden md:flex items-center gap-2 max-w-sm w-full mx-8">
                        <div className="relative w-full">
                            <Search className="absolute left-3.5 top-1/2 -translate-y-1/2 text-gray-400" size={16} />
                            <input 
                                type="text"
                                placeholder="Search symbol (e.g. INFY.NS, AAPL)..."
                                value={searchSymbol}
                                onChange={(e) => setSearchSymbol(e.target.value)}
                                className="w-full pl-10 pr-4 py-2 bg-white/5 rounded-xl border border-white/10 focus:outline-none focus:border-cyan-400 text-sm font-semibold tracking-wide"
                            />
                        </div>
                        <button type="submit" className="px-4 py-2 bg-cyan-500 hover:bg-cyan-400 text-black font-black text-sm rounded-xl transition">
                            Search
                        </button>
                    </form>

                    <button onClick={onClose} className="p-2 text-gray-400 hover:text-white rounded-xl hover:bg-white/5 transition">
                        <X size={20} />
                    </button>
                </div>

                {/* Search Bar Mobile */}
                <div className="p-4 border-b border-white/5 bg-white/[0.005] md:hidden">
                    <form onSubmit={handleSearch} className="flex gap-2">
                        <input 
                            ...
                            value={searchSymbol}
                            onChange={(e) => setSearchSymbol(e.target.value)}
                            className="w-full px-4 py-2 bg-white/5 rounded-xl border border-white/10 focus:outline-none focus:border-cyan-400 text-sm"
                        />
                        <button type="submit" className="px-4 py-2 bg-cyan-500 text-black font-black text-sm rounded-xl">
                            Go
                        </button>
                    </form>
                </div>
                    ...
                    {loading ? (
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                            ...
                            <div className="lg:col-span-2 space-y-8">
                                ...
                                {/* Live Feed Header */}
                                <div className="glass p-6 rounded-3xl border border-white/5 bg-gradient-to-r from-cyan-500/5 to-transparent flex flex-wrap justify-between items-center gap-4">
                                    <div>
                                        <div className="text-[10px] font-black uppercase tracking-widest text-cyan-400 mb-1">Live Stock Intelligence</div>
                                ...
                                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                    ...
                                    <div className="glass p-4 rounded-2xl border border-white/5 bg-white/[0.01]">
                                        ...
                                        <div className="text-lg font-black text-cyan-400">{currentData.trend_strength}</div>
                                    </div>
                                    <div className="glass p-4 rounded-2xl border border-white/5 bg-white/[0.01]">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    ...
                                    {/* 2-Hour Intraday */}
                                    <div className="glass p-6 rounded-3xl border border-white/5 bg-gradient-to-b from-white/[0.02] to-transparent space-y-4">
                                ...
                                <div className="glass p-6 rounded-3xl border border-cyan-500/10 bg-gradient-to-br from-cyan-500/5 to-transparent relative overflow-hidden">
                                    <div className="absolute right-0 top-0 translate-x-4 -translate-y-4 p-8 bg-cyan-500/10 rounded-full blur-2xl" />
                                    <div className="flex items-center gap-2 mb-3">
                                        <Sparkles className="text-cyan-400 animate-pulse" size={20} />
                                        <h4 className="font-black text-sm uppercase tracking-wider text-cyan-400">Explainable AI Justification</h4>
                                    </div>
                                    <p className="text-gray-300 text-sm leading-relaxed tracking-wide font-medium">
                                        "{currentData.predictions["2h"].explanation}"
                                    </p>
                                    ...
                                    <div className="mt-6 flex flex-col md:flex-row justify-between items-stretch md:items-center gap-4 pt-4 border-t border-white/5">
                                        ...
                                        <div className="text-[10px] text-gray-500 font-mono text-right flex items-center justify-end gap-1.5">
                                            ...
                                            Active Model Checkpoint: river.forest.ARFClassifier
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    <div className="flex justify-between items-center mb-6">
                                        ...
                                        <div className="flex items-center gap-2">
                                            ...
                                            <span className={`text-xs font-black px-2.5 py-0.5 rounded border ${
                                                currentData.sentiment.classification === "POSITIVE" 
                            ...
                            {/* Right Col: Incremental trainer control, picks, prediction history */}
                            <div className="space-y-8">
                                
                                {/* Advanced Feature 1: Top Picks Card */}
                                    ...
                                    <div className="space-y-3">
                                        {topPicks.length > 0 ? (
                                            ...
                                            <div className="p-4 bg-white/5 border border-white/5 rounded-xl text-center text-xs text-gray-500 italic">
                                                Scanning market watchlists... (Requires high-confidence BUY signals)
                                            </div>
                                        )}
                                    </div>
                                </div>
                                ...
                                <div className="glass p-6 rounded-3xl border border-white/5 bg-white/[0.01] space-y-4">
                                    ...
                                    <div className="grid grid-cols-2 gap-4">
                                        <div className="p-3 bg-white/5 rounded-xl text-center border border-white/5">
                                            ...
                                            <div className="text-2xl font-black text-cyan-400">{(currentData.metrics.accuracy * 100).toFixed(0)}%</div>
                                        </div>
                                        <div className="p-3 bg-white/5 rounded-xl text-center border border-white/5">
                                            <div className="text-[10px] text-gray-500 font-bold uppercase tracking-wider">Total Samples</div>
                                            <div className="text-2xl font-black text-cyan-400">{currentData.metrics.total_predictions}</div>
                                        </div>
                                        <div className="p-3 bg-white/5 rounded-xl text-center border border-white/5">
                                ...
                                {/* Incremental update engine controls */}
                                <div className="glass p-6 rounded-3xl border border-cyan-500/20 bg-gradient-to-b from-cyan-500/10 to-transparent space-y-4">
                                    <div className="flex justify-between items-center">
                                        <span className="text-xs font-black uppercase tracking-widest text-cyan-400">Incremental Trainer</span>
                                        <span className="text-[9px] font-mono text-gray-500 bg-white/5 px-2 py-0.5 rounded">Continuous Feed</span>
                                    </div>
                                    
                                    <p className="text-xs text-gray-400 leading-normal">
                                        Demonstrate online learning. Download latest candles, scrape financial news, evaluate past predictions, and run River training step.
                                    </p>

                                    <button 
                                        onClick={triggerIncrementalUpdate}
                                        disabled={updatingModel}
                                        className="w-full py-3 bg-cyan-500 hover:bg-cyan-400 disabled:bg-cyan-800 text-black font-black rounded-xl transition flex items-center justify-center gap-2 text-sm shadow-lg shadow-cyan-500/20 active:scale-95"
                                    >
                                        <RefreshCw size={16} className={updatingModel ? "animate-spin" : ""} />
                                        {updatingModel ? "TRAINING ARF MODEL..." : "TRIGGER ONLINE UPDATE"}
                                    </button>

                                    {/* Real-time Update Logger */}
                                    {(updatingModel || updateLogs.length > 0) && (
                                        <div className="p-4 bg-black/60 rounded-xl border border-white/5 font-mono text-[9px] text-cyan-400 space-y-1.5 max-h-[150px] overflow-y-auto">
                                ...
                                {/* Advanced Feature 2: Prediction Timeline History */}
                                <div className="glass p-6 rounded-3xl border border-white/5 bg-white/[0.005]">
                                    <div className="flex justify-between items-center mb-4">
                                        <div className="flex items-center gap-2">
                                            <Clock size={16} className="text-gray-400" />
                                            <h4 className="font-bold text-sm uppercase tracking-wider">Prediction History Timeline</h4>
                                        </div>
                                        ...
                                        {timeline.length > 0 ? (
                                            timeline.map((item: any, idx: number) => (
                                                <div key={idx} className="relative pl-6 border-l border-white/10 pb-4 last:pb-0">
                                                    {/* Timeline Dot */}
                                                    <span className={`absolute left-0 -translate-x-1/2 top-1.5 h-3.5 w-3.5 rounded-full border-2 border-[#090d22] flex items-center justify-center ${
                                                        item.status === "correct" 
                        ...
                        <div className="text-center py-24 text-gray-500">
                            Select a stock or search above to load predictive metrics.
                        </div>
                    )}
                </div>
            </div>
